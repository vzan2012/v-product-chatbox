...
const parseGroqResponse = (groqText: string, images: ImageFileState[]) => {
...
export const analyzeBatchImages = async (
  ...
  try {
    const chatModel = callGroqVisionModel();
      ...
      new HumanMessage({
        content: [
          ...
          ...images.map((image) => ({
            ...
            image_url: { url: image.base64Data },
          })),
        ],
      }),
    ];
    ...
    const results = parseGroqResponse(responseText, images);

    return { query, results };
  } catch (error) {
    console.error("Groq API Error:", error);
    throw new Error("Failed to analyze images with Groq. Please try again.");
  }
};

export const chatWithGroq = async (query: string): Promise<string> => {
  try {
    const chatModel = callGroqTextModel();
    ...
    console.error("Groq text chat error:", error);
